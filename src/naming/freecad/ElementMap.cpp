...
// App/Application.h removed
#include "compat/BaseStubs.h"
// Document.h removed
// DocumentObject.h removed
...
{
    // Standalone mode: no FreeCAD document signals.
}
    ...
    for (int index = 0; index < 50; ++index) {
        ...
        if (encodedTag && masterTag != std::abs(encodedTag)
            ...
            if (FC_LOG_INSTANCE.isEnabled(FC_LOGLEVEL_LOG)) {
                ...
                if (FC_LOG_INSTANCE.isEnabled(FC_LOGLEVEL_TRACE)) {
                    // App::GetApplication removed — standalone mode
                    // (debug document lookup not available)
                    //
                    //
                    //
                    //
